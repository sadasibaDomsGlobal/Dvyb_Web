...
const Save = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4 py-10 z-50 overflow-y-auto">
      {/* Modal Container */}
      <div className="relative  flex flex-col md:flex-row max-w-4xl w-full rounded-lg overflow-hidden shadow-2xl p-6 md:p-10">
        {/* Close Button */}
        <button
          onClick={closeModal}
          className="absolute top-10  right-7 sm:top-14 sm:right-14 text-[#5A2E14] hover:text-[#7A3E1B] focus:outline-none z-10 cursor-pointer"
        ...
        <div className="w-full md:w-1/2 bg-[#FDF2EA] p-6 md:p-10 flex flex-col items-center justify-center text-center">
          ...
          {/* Description */}
          <p className="text-base text-[#5A2E14] mb-8 max-w-sm">
