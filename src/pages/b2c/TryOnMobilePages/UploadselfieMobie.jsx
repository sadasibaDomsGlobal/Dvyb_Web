...
const UploadSelfieModalMobile = ({ isOpen, onClose, onNext, garmentImage, garmentName, isSaree, is3D = false }) => {
...
<div className="flex flex-col md:flex-row bg-white md:rounded-xl md:gap-[26px] overflow-hidden md:shadow-lg w-full md:w-[838px] h-auto md:h-[564px]">
    ...
    {/* LEFT IMAGE SECTION */}
    <div className="flex justify-center items-center md:p-3 bg-black md:bg-transparent">
      <img
        src={step1img}
        alt="Garment"
        className="w-full h-[320px] md:w-[400px] md:h-[532px] object-cover"
      />
    </div>

    {/* RIGHT SIDE CONTENT */}
    <div className="flex flex-col px-4 py-5 md:pt-16 md:px-0 md:w-[400px] w-full pb-20 md:pb-0 relative bg-white">
      
      {/* HEADING - Shows first on mobile, after card on desktop */}
      <div className="block md:hidden mb-4">
        <h2 className="text-[22px] font-bold text-black">Try-On</h2>
        <p className="text-gray-500 text-[13px] mt-0.5 font-normal">Let's go shopping</p>
      </div>

      {/* SELECTED DRESS CARD */}
      <div className="w-full md:w-[180px] rounded-lg shadow-sm border p-2.5 border-gray-200 mb-5 md:mb-0">
      ...
      <button
        onClick={() => setStep(2)}
        className="w-full md:w-[345px] bg-[#8B0000] text-white h-[46px] text-[13px] font-medium hover:bg-[#A30000] transition rounded md:rounded-none"
      >
        Upload a picture
      </button>
      ...
      <button
        onClick={onClose}
        className="w-full md:w-[345px] border border-[#8B0000] text-[#8B0000] h-[46px] text-[13px] font-medium mt-3 hover:bg-[#8B0000] hover:text-white transition rounded md:rounded-none"
      >
        Select a model
      </button>

      {/* FOOTNOTE */}
      <p className="text-[10px] mt-4 text-gray-500 leading-[1.4] md:max-w-[320px] text-start">
        Hey! To use the 2D TRY ON feature, just upload or take a selfie or <br className="hidden md:inline" />Press <span className='text-[#8B0000] text-[10px] font-semibold'>SKIP</span> to check out the models you can try on!
      </p>

      {/* SKIP BUTTON (BOTTOM-RIGHT) */}
      <button
        onClick={onClose}
        className="absolute bottom-5 right-4 cursor-pointer text-gray-600 border border-gray-300 px-3.5 py-1.5 rounded text-[12px] font-medium flex items-center gap-1.5 hover:text-gray-800 hover:border-gray-400"
      >
        SKIP
